...
class BaseVLNCETrainer(BaseILTrainer):
    ...
    def _eval_checkpoint_bak(
        ...
        while self.envs.num_envs > 0 and len(stats_episodes) < episodes_to_eval:
            ...
            #h_t, language_features, language_attention_mask, token_type_ids, current_episodes = curr_r2r_agent.encode_instruction()

            # --------
            for t in range(3000):

                #actions = curr_r2r_agent.predict_next_step(h_t, language_features, language_attention_mask, token_type_ids)
