...
class r2r_agent():
    ...
    def train_listener(self, train_ml=None, train_rl=True, reset=True, **kwargs):
        ...
        for epoch in range(self.config.R2R_MODEL.epoch):
            ...
            self.observations = self.envs.reset()
            self.constant_num_envs = self.envs.num_envs
            ...
            episodes_num = sum(self.envs.number_of_episodes)
            end_iter = episodes_num // self.envs.num_envs

            logger.info(f"start_iter, {start_iter}")

            for idx in range(start_iter, end_iter , self.config.R2R_MODEL.log_every):
                interval = min(self.config.R2R_MODEL.log_every, end_iter - idx)
                #iter = idx + interval

                #jdx_length = len(range(interval // 2))
                #for jdx in range(interval // 2):
                for jdx in range(interval):

                    # Train with GT data
                    #args.ml_weight = 0.2
                    #self.train(1, feedback=feedback_method)
                    ijdx = idx * self.config.R2R_MODEL.log_every + jdx
                    self.train(self.envs, self.config, train_ml=None, train_rl=True, reset=True, ijdx = ijdx , **kwargs)

                    print_progress(jdx, self.config.R2R_MODEL.log_every + 1, prefix='Progress:', suffix='Complete', bar_length=50)
                ...
                """
                #set_trace()
                #print(self.config.SAVE_NAME)
                self.save(idx, os.path.join("snap", self.config.R2R_MODEL.name, self.config.SAVE_NAME, "LAST_iter%d_%d_" % (epoch,idx)))
    ...
    def encode_instruction(self):
        ...
        reward , dones , info = self.reset_observations()
        ...
        return h_t, language_features, language_attention_mask, token_type_ids, current_episodes, dones, info

    def clear_rl_candidate(self):
        self.rl_input_a_t = None
        self.rl_candidate_feat = None
        self.rl_candidate_leng = None
        self.rl_candidates_mask = None

    def get_rl_candidate(self, input_a_t, candidate_feat, candidate_leng, candidates_mask):
        if self.rl_input_a_t == None:
            self.rl_input_a_t = input_a_t.clone()
            self.rl_candidate_feat = candidate_feat.clone()
            self.rl_candidate_leng = candidate_leng.copy()
            self.rl_candidates_mask = candidates_mask.clone()

        for index in range(len(self.dones_mask)):
            if self.dones_mask[index] == True:
                self.rl_input_a_t[index] = input_a_t[index].clone()
                self.rl_candidate_feat[index] = candidate_feat[index].clone()
                self.rl_candidate_leng[index] = candidate_leng[index].copy()
                self.rl_candidates_mask[index] = candidates_mask[index].clone()
    ...
    def predict_next_step(self, h_t, language_features, language_attention_mask, token_type_ids):
        ...
        self.get_rl_candidate(input_a_t, candidate_feat, candidate_leng, candidates_mask)
    ...
    def _eval_checkpoint(
        ...
        self.dones_mask = np.array([True for _ in range(self.constant_num_envs)])
        ...
        for i in range(458): #458
            ...
            h_t, language_features, language_attention_mask, token_type_ids, current_episodes, dones, info = self.encode_instruction()
            #print("i", i)
            self.clear_rl_candidate()
            # --------
    ...
    def train(self, envs, config, train_ml=None, train_rl=True, reset=True, ijdx=0, **kwargs):
        self.envs = envs
        feedback = "sample"
        n_iters = 1
        ...
        for iter in range(1, n_iters + 1):
            ...
            if ijdx % 2 == 0:
                ...
                #print(ijdx , "teacher")
                self.rollout(envs, config, train_ml=self.config.R2R_MODEL.ml_weight, train_rl=False, **kwargs)
            else:
                self.feedback = 'sample'
                #print(ijdx , "sample")
                self.rollout(envs, config, train_ml=None, train_rl=True, **kwargs)
            ...
            if feedback == 'teacher':
                ...
                if args.ml_weight != 0:
                    ...
                    self.rollout(envs, config, train_ml=args.ml_weight, train_rl=False)

                self.feedback = 'sample'
                #self.rollout(train_ml=None, train_rl=True, **kwargs)
                self.rollout(envs, config, train_ml=None, train_rl=True)
    ...
    def rollout(self, envs, config, train_ml=None, train_rl=True, reset=True):
        ...
        for i in range(rollout_count):
            ...
            hidden_states = []
            ended = np.array([False] * self.constant_num_envs)
            ...
            for t in range(self.config.R2R_MODEL.max_step):
                ...
                target = np.zeros((self.constant_num_envs), np.longlong)
                mask_index_count = 0
                if self.envs.num_envs < self.constant_num_envs:
                    for mask_index in range(len(self.dones_mask)):
                        if self.dones_mask[mask_index]:
                            target[mask_index] = self.observations[mask_index][expert_uuid]
                        elif not  self.dones_mask[mask_index]:
                            target[mask_index] = self.config.R2R_MODEL.ignoreid
                ...
                for i, next_id in enumerate(cpu_a_t):
                    ...
                    if next_id == 0:
                        #logger.info(f"predict 0: {cpu_a_t}")
                        cpu_a_t[i] = 0
                    if next_id == args.ignoreid or ended[i]:  # The last action is <end>
                        cpu_a_t[i] = args.ignoreid

                actions = torch.tensor(cpu_a_t.reshape(cpu_a_t.shape[0], 1), device=self.device)
                ...
                """
                if self.envs.num_envs < self.constant_num_envs:
                    actions = [[item] for item in actions[self.dones_mask] if item.item() != -100]
                    actions = torch.tensor(actions, device=self.device)
                ...
                for index in range(len(dones)):
                    if dones[index]:
                        self.dones_mask[index] = False
                        envs.pause_at(index)
                """

                dones_ori = dones
                if len(dones) < self.constant_num_envs:
                    #dones_copy = [True for _ in range(len(self.constant_num_envs))]
                    dones_copy = [True if item == False else False for item in self.dones_mask]
                    item_count = 0
                    for item_index in range(len(dones_copy)):
                        if dones_copy[item_index] == False:
                            dones_copy[item_index] = dones[item_count]
                            item_count += 1
                    dones = dones_copy

                if self.envs.num_envs < self.constant_num_envs and len(info) < self.constant_num_envs:
                    info_ori = info.copy()
                    while len(info) < len(self.dones_mask):
                        info.append(info[0])
                    index_flag_count = 0
                    for index_flag in range(len(self.dones_mask)):
                        if self.dones_mask[index_flag] == True:
                            info[index_flag] = info_ori[index_flag_count]
                            index_flag_count += 1

                dones_index = len(dones_ori) - 1
                for index in range(len(self.dones_mask)-1, -1, -1):
                    if self.dones_mask[index] == True:
                        if dones_ori[dones_index]:
                            #logger.info(f"dones true: {dones_ori}")
                            self.dones_mask[index] = False
                            self.envs.pause_at(dones_index)
                        dones_index -= 1


                if train_rl:
                    # Calculate the mask and reward
                    dist = np.zeros(self.constant_num_envs, np.float32)
                    ndtw_score = np.zeros(self.constant_num_envs, np.float32)
                    reward = np.zeros(self.constant_num_envs, np.float32)
                    mask = np.ones(self.constant_num_envs, np.float32)
                    for i in range(self.constant_num_envs):
                        if self.dones_mask[i] == False:
                            continue
                ...
                ended[:] = np.logical_or(ended, np.logical_or((cpu_a_t == 0), (cpu_a_t == -100)))

                # Early exit if all ended
                if ended.all() or self.envs.num_envs <= 0:
                    #logger.info(f"end all true: {ended}, {self.envs.num_envs}")
                    self.envs.resume_all()
                    self.dones_mask = np.array([True for _ in range(self.constant_num_envs)])
                    break

                if t == self.config.R2R_MODEL.max_step - 1:
                    #logger.info(f"max step: {t},{self.envs.num_envs}")
                    index_arr = [[0] for _ in range(self.envs.num_envs)]
            ...
            if train_rl:
                ...
                input_a_t = self.rl_input_a_t
                candidate_feat = self.rl_candidate_feat
                candidate_leng = self.rl_candidate_leng
                candidates_mask = self.rl_candidates_mask
            ...
            if type(self.loss) is int:  # For safety, it will be activated if no losses are added
                ...
                self.losses.append(self.loss.item() / self.config.R2R_MODEL.max_step)  # This argument is useless.




    def get_input_feat(self, env, curr_elevation):
        input_a_t = np.zeros((self.constant_num_envs , self.config.R2R_MODEL.angle_feat_size), np.float32)
    ...
    def encode_instr(self, instr):
        ...
        padded_instr_tokens, num_words = self.pad_instr_tokens(instr_tokens, self.config.R2R_MODEL.max_instruction_length)
        instr_encoding = self.tokenizer.convert_tokens_to_ids(padded_instr_tokens)
    ...
    def make_candidates(self):
        ...
        self.candidates_mask = torch.tensor([[1 for i in range(action_num)] for _ in range(self.constant_num_envs)]).to(self.device)
        self.candidate_leng = np.array([[action_num] for _ in range(self.constant_num_envs)])
        self.candidates = torch.ones(
            self.constant_num_envs,
            action_num,
    ...
    def go_step(self, index, heading, elevation, up_down_flag, need_return = False):
        ...
        index_arr = [[index] for _ in range(self.constant_num_envs)]
        """
        ...
        if index == 1:
            ...
            for index in range(len(actions)):
                if self.dones_mask[index] == True:
                    actions[index][0] = 0
            """
            actions = actions[self.dones_mask]
            outputs = self.envs.step([a[0].item() for a in actions])
            observations, _, dones, _ = [list(x) for x in zip(*outputs)]
            #print("get_done", self.envs.call_at(0, "get_done", {"observations": self.observations}))

        if self.envs.num_envs < self.constant_num_envs and len(observations) < self.constant_num_envs:
            observations_ori = observations.copy()
            while len(observations) < len(self.dones_mask):
                observations.append(observations[0])
            index_flag_count = 0
            for index_flag in range(len(self.dones_mask)):
                if self.dones_mask[index_flag] == True:
                    observations[index_flag] = observations_ori[index_flag_count]
                    index_flag_count += 1
        ...
        if need_return:
            ...
            feature = torch.ones((self.constant_num_envs, self.config.R2R_MODEL.image_feature_size + self.config.R2R_MODEL.angle_feature_size))*(-1)
            #for i in range(self.envs.num_envs):

            image_feature = self.get_image_features(observations)
            #image_feature = self.get_image_features(observations[i]["rgb"])
            feature = torch.cat((image_feature, torch.cat([angle_feature for i in range(self.constant_num_envs)] , dim = 0)), dim=1)
            return feature
